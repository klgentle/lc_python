...
public class SendMail {
	...
	public static void main(String[] args) throws Exception {
		...
		final String password = "dksf"; // need to modify
...
//		Session session = sender.createSSLSession();
		Message message = createTextMessage(session, from, to, "JavaMail邮件", "你好, 这是一封来自javamail的邮件！");
	...
	Session createTLSSession() {
		...
		props.put("mail.smtp.ssl.trust", "smtp.sina.com");  // 添加 ssl.trust
		Session session = Session.getInstance(props, new Authenticator() {
			protected PasswordAuthentication getPasswordAuthentication() {
				return new PasswordAuthentication(SendMail.this.username, SendMail.this.password);
			}
