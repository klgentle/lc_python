...
def mail():
    ...
    try:
        ...
        message['To'] = formataddr(['Dear', addressed_eamil])  # 括号里的对应收件人邮箱昵称、收件人邮箱账号
        #message['To'] = formataddr(['收件人姓名', ','.join(addressed_eamil)])  # 括号里的对应收件人邮箱昵称、收件人邮箱账号
        ...
        #server.sendmail(sender, ','.join(addressed_eamil2), message.as_string())  # 括号中对应的是发件人邮箱账号、收件人邮箱账号、发送邮件
        server.quit()  # 关闭连接
    except Exception:
        print("邮件发送失败")
